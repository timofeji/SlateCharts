 ...
 */
#if WITH_EDITOR
inline TArray<FVector2D> GenerateMockTimeSeriesData(float SampleRate, float Amplitude)
{
	TArray<FVector2D> MockData;

	int32 n = (2 * PI / SampleRate);

	for (int32 Index = -n; Index < n; ++Index)
	{
		const float Value = Amplitude * FMath::Sin(Index * SampleRate);
		MockData.Add(FVector2D(Index, Value));
	}
...
#if WITH_EDITOR
	SetData(GenerateMockTimeSeriesData(.05f, 40.f));
	FString Expression = TEXT("5 * 7 - 3 / 8 + 2");
	float Result = EvaluateExpression(Expression);
	UE_LOG(LogTemp, Log, TEXT("The result of the expression is: %f"), Result);
#endif
}
